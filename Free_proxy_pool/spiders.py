...
class Proxy_Spider(object):
    ...
    def crawl_for_init(self):
        self.__get_jiangxianli()     # 数量自定义。抓取到的代理越多，验证用时越长。
    ...
    def __ihuan(self):
        urls = [
            'https://ip.ihuan.me/?page={}&anonymity=2'.format(str(i)) for i in range(1, 10)]
        for url in urls:
            try:
                ...
                print('小幻代理 获取失败', end='\t\r')
            print('已采集小幻代理，代理池IP总数：', len(self.proxies_got))

    # 正则提取， 66ip、89ip、QQ_room、开心代理采用了这种解析方式
    def __parse_by_re(self, url, reg_exp=re.compile('\w+\.\w+\.\w+\.\w+:\w+')):
        article = None if self.request_page(
            url) is None else self.request_page(url).__unicode__()
    ...
    def __get_All66(self):
        ...
        for url in urls:
            try:
                self.proxies_got.update(
                    self.__parse_by_re(url))  # 把找到的代理IP添加到集合里面
            except Exception:
                print('66ip.cn 获取失败', end='\t\r')
            print('已采集66ip.cn，代理池IP总数：', len(self.proxies_got))
            time.sleep(1.1)

    def __get_jiangxianli(self):
        print('开始采集ip.jiangxianli.com')
        urls = ['https://ip.jiangxianli.com/?page={}'.format(str(i)) for i in range(1, 3)]
        # 采集国内的，高匿、超匿2种HTTP代理。如果想采集国外的，area改为2。【如果要采集HTPPS，proxytpye = 1 】
        for url in urls:
            try:
                for info in self.__rows_from(url):
                ...
                print('ip.jiangxianli.com 获取失败', end='\t\r')
            print('已采集ip.jiangxianli.com，代理池IP总数：', len(self.proxies_got))
            time.sleep(30)
    ...
    def __space(self):
        ...
        try:
            soup = self.request_page('https://openproxy.space/list/http')
            item = soup.select('div.list-code textarea.text-input')
            address = item[0].text
            print(address)
        except Exception:
            print('openproxy.space 获取失败', end='\t\r')
        time.sleep(0.5)
    ...
    def __kaixin(self):
        ...
        # 只抓取首页的前2篇文章
        for info in soup.select('div.cont_list a.title')[:1]:
    ...
    def test(self):
        ...
        self.__get_jiangxianli() 


if __name__ == '__main__':
    spider = Proxy_Spider()
    spider.crawl_for_init()
